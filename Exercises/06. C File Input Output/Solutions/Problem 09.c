...
const char* ERROR_FILE_P = "Error while operating with the file - CORRUPTED!";
...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(cmnd, "--min") == 0) {
        ...
        err(666, "Expected Different Argument!");
    }
...
void executeMinCommand(const char* filePath) {
    ...
    uint16_t number = 0;    readingSafe(filePath, descriptor, &number, sizeof(uint16_t));

    for (off_t i = 1; i < size; ++i) {
        uint16_t current = 0;
        
        readingSafe(filePath, descriptor, &current, sizeof(uint16_t));
