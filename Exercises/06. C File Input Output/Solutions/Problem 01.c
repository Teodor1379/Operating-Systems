...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        if (readingSafe(file1, descriptors[0], &c, sizeof(char)) == 0) {
            break;
        }

        writingSafe(file2, descriptors[1], &c, sizeof(c));
    }
...
void openingSafe(const char* filePath, int flags, mode_t mode, int* memory) {
    assert(filePath !=  NULL);
    assert(memory   !=  NULL);

    *memory = open(filePath, flags, mode);
